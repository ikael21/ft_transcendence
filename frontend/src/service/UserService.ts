...
class UserService {
  async fetchJwt(code: string, bypassId?: string) {
    return axios
      .get(`http://${import.meta.env.VITE_DOMAIN}:3001/42intra/callback`, {
  ...
  async generate2FA(): Promise<string> {
    return axios
      .get(`http://${import.meta.env.VITE_DOMAIN}:3001/2fa/generate`, {
        withCredentials: true,
        responseType: 'blob',
      })
      .then((res) => {
  ...
  async verify2FA(code: string): Promise<void> {
    return axios
      .get(`http://${import.meta.env.VITE_DOMAIN}:3001/2fa/verify`, {
        params: { code },
        withCredentials: true,
      })
      .then(() => {
  ...
  async disable2FA(): Promise<void> {
    return axios
      .get(`http://${import.meta.env.VITE_DOMAIN}:3001/2fa/disable`, {
        withCredentials: true,
      })
      .then(() => {
        return;
  ...
  async enable2FA(code: string) {
    return axios
      .get(`http://${import.meta.env.VITE_DOMAIN}:3001/2fa/enable`, {
        params: { code },
        withCredentials: true,
      })
      .then(() => {
  ...
  async uploadPicture(file: File): Promise<string> {
    ...
    return axios
      .post(
        `http://${import.meta.env.VITE_DOMAIN}:3001/users/upload`,
        formData,
        {
          withCredentials: true,
        },
  ...
  async logout(): Promise<void> {
    await axios
      .get(`http://${import.meta.env.VITE_DOMAIN}:3001/42intra/logout`, {
        withCredentials: true,
      })
      .then(() => {
        return;
